...
new TimeSeries({
  name: "valid",
  schema,
  rows: [[new Date(), 1, "x"], [new Time(Date.now()), 2, "y"]],
});

const jsonTimestamp: JsonTimestampInput = "2025-01-01T09:00";
const jsonTimeRange: JsonTimeRangeInput = { start: "2025-01-01", end: "2025-01-02" };
...
const parsedTime = Time.parse("2025-01-01T09:00", { timeZone: "Europe/Madrid" });
const parsedDayRange = TimeRange.fromDate("2025-01-01", { timeZone: "UTC" });
const parsedWeekRange = TimeRange.fromCalendar("week", "2025-01-01", { timeZone: "UTC", weekStartsOn: 1 });
const parsedMonthInterval = Interval.fromCalendar("month", "2025-01", { timeZone: "UTC", value: "jan-2025" });
void parsedTime;
void parsedDayRange;
...
const eventOverlaps: boolean = nth.overlaps(new TimeRange({ start: 1735689599000, end: 1735689601000 }));
...
const eventTrimmed = nth.trim(new TimeRange({ start: 1735689599000, end: 1735689601000 }));
...
const collapsedEvent = nth.collapse(["cpu", "healthy"], "score", ({ cpu, healthy }) => {
...
const collapsedSeries = trafficSeries.collapse(["in", "out"], "avg", ({ in: inValue, out }) => {
...
const boundedMinuteSequence = new BoundedSequence([
  new Interval({ value: 1735689600000, start: 1735689600000, end: 1735689660000 }),
]);
...
const alignedTypedSchemaEvent: EventForSchema<AlignedCpuSchema> = alignedCpuEvent;
const cpuSeriesAsTimeRangeTypedEvent: EventForSchema<CpuAsTimeRangeSchema> = cpuSeriesAsTimeRangeEvent;
const cpuSeriesAsIntervalTypedEvent: EventForSchema<CpuAsIntervalSchema> = cpuSeriesAsIntervalEvent;
void alignedCpuKey;
...
type AggregatedCpuSchema = AggregateSchema<typeof cpuSchema, {
  readonly cpu: "avg";
  readonly host: "first";
...
const aggregatedCpuHealthy: boolean | undefined = aggregatedCpuEvent.get("healthy");
const aggregatedTypedSchemaEvent: EventForSchema<AggregatedCpuSchema> = aggregatedCpuEvent;
...
const rolledCpuSeries = cpuSeries.rolling("1m", { cpu: "avg", host: "last", healthy: "last" });
type RolledCpuSchema = RollingSchema<typeof cpuSchema, {
  readonly cpu: "avg";
  readonly host: "last";
  readonly healthy: "last";
}>;
...
type RolledCpuSequenceSchema = AggregateSchema<typeof cpuSchema, {
  readonly cpu: "avg";
  readonly host: "last";
  readonly healthy: "last";
}>;
...
const rolledCpuOnSequenceValue: number | undefined = rolledCpuOnSequenceEvent.get("cpu");
const rolledCpuOnSequenceHost: string | undefined = rolledCpuOnSequenceEvent.get("host");
const rolledCpuOnSequenceHealthy: boolean | undefined = rolledCpuOnSequenceEvent.get("healthy");
const rolledCpuOnSequenceTypedEvent: EventForSchema<RolledCpuSequenceSchema> = rolledCpuOnSequenceEvent;
...
const smoothedCpuTypedEvent: EventForSchema<SmoothedCpuSchema> = smoothedCpuEvent;
void smoothedCpuKey;
...
const appendedSmoothedCpuSeries = cpuSeries.smooth("cpu", "ema", { alpha: 0.5, output: "cpuEma" });
...
const appendedSmoothedCpuOutput: number | undefined = appendedSmoothedCpuEvent.get("cpuEma");
const appendedSmoothedCpuHost: string = appendedSmoothedCpuEvent.get("host");
const appendedSmoothedCpuTypedEvent: EventForSchema<AppendedSmoothedCpuSchema> = appendedSmoothedCpuEvent;
...
const loessSmoothedCpuSeries = cpuSeries.smooth("cpu", "loess", { span: 0.75, output: "cpuLoess" });
...
const loessSmoothedCpuOutput: number | undefined = loessSmoothedCpuEvent.get("cpuLoess");
const loessSmoothedCpuTypedEvent: EventForSchema<LoessSmoothedCpuSchema> = loessSmoothedCpuEvent;
...
const joinedLeftSeries = alignedCpuSeries.join(alignedHostSeries, { type: "left" });
const joinedInnerSeries = alignedCpuSeries.join(alignedHostSeries, { type: "inner" });
const prefixedJoinedAlignedSeries = alignedCpuSeries.join(alignedHostSeries, {
  onConflict: "prefix",
  prefixes: ["cpu", "host"] as const,
});
type JoinedAlignedSchema = JoinSchema<AlignSchema<typeof cpuSchema>, AlignSchema<typeof hostSchema>>;
...
const joinedAlignedHealthy: boolean | undefined = joinedAlignedEvent.get("healthy");
const joinedAlignedTypedEvent: EventForSchema<JoinedAlignedSchema> = joinedAlignedEvent;
void joinedAlignedKey;
void joinedAlignedCpu;
void joinedAlignedHost;
void joinedAlignedHealthy;
void joinedAlignedTypedEvent;
const prefixedJoinedAlignedCpu: number | undefined = prefixedJoinedAlignedEvent.get("cpu");
const prefixedJoinedAlignedHost: string | undefined = prefixedJoinedAlignedEvent.get("host_host");
const prefixedJoinedAlignedHealthy: boolean | undefined = prefixedJoinedAlignedEvent.get("healthy");
const prefixedJoinedAlignedTypedEvent: EventForSchema<PrefixedJoinedAlignedSchema> = prefixedJoinedAlignedEvent;
...
const statusSeries = new TimeSeries({
  name: "status",
  schema: statusSchema,
  rows: [[new Interval({ value: 1735689600000, start: 1735689600000, end: 1735689660000 }), "ok"]],
});

const joinedManySeries = TimeSeries.joinMany([alignedCpuSeries, alignedHostSeries, statusSeries]);
const prefixedJoinedManySeries = TimeSeries.joinMany([alignedCpuSeries, alignedHostSeries, statusSeries], {
  onConflict: "prefix",
  prefixes: ["cpu", "host", "status"] as const,
});
type JoinedManyAlignedSchema = JoinManySchema<
  readonly [AlignSchema<typeof cpuSchema>, AlignSchema<typeof hostSchema>, typeof statusSchema]
>;
...
const joinedManyTypedEvent: EventForSchema<JoinedManyAlignedSchema> = joinedManyEvent;
void joinedManyKey;
void joinedManyCpu;
void joinedManyHost;
...
const prefixedJoinedManyCpu: number | undefined = prefixedJoinedManyEvent.get("cpu");
const prefixedJoinedManyHost: string | undefined = prefixedJoinedManyEvent.get("host_host");
const prefixedJoinedManyHealthy: boolean | undefined = prefixedJoinedManyEvent.get("healthy");
const prefixedJoinedManyStatus: string | undefined = prefixedJoinedManyEvent.get("status");
const prefixedJoinedManyTypedEvent: EventForSchema<PrefixedJoinedManyAlignedSchema> = prefixedJoinedManyEvent;
