...
describe("Time", () => {
  it("constructs a point key and exposes its temporal API", () => {
    ...
    expect(time.timeRange()).toEqual(new TimeRange({ start: 1735689600000, end: 1735689600000 }));
  ...
  it("supports temporal relation and trim operations", () => {
    ...
    expect(time.intersection(containing)).toEqual(new TimeRange({ start: 1000, end: 1000 }));
  ...
  it("parses local and absolute timestamp strings with timezone awareness", () => {
    expect(Time.parse("2025-01-01T09:00", { timeZone: "Europe/Madrid" })).toEqual(
