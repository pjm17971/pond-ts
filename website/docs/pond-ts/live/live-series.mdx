...
schema type system as [`TimeSeries`](../concepts/overview#timeseries).
...
pond-ts is batch-first: `LiveSeries` is a bounded in-order buffer with
an opt-in grace window for moderate reordering at ingest, not a full
streaming engine. For the specifics of what that does and doesn't
cover once data leaves the buffer, see
...
doesn't model live data. The closest analogue is a bounded deque you
periodically `.toTimeSeries()` on and run batch methods against; that
"snapshot-then-batch" is the pandas-shaped escape hatch from the live
side.

If you're coming from pondjs: `LiveSeries` takes over `Pond`'s
live-collection role, with retention baked in instead of bolted on.

| pondjs                   | pond-ts                                        |
| ------------------------ | ---------------------------------------------- |
| `Pond` (live store)      | `LiveSeries`                                   |
| `Aggregator`             | `LiveAggregation`                              |
| custom rolling processor | `LiveRollingAggregation`                       |
| manual eviction on push  | `retention: { maxEvents / maxAge / maxBytes }` |
| `Pipeline` for live      | method chain on `LiveSeries` / `LiveView`      |
...
const live = new LiveSeries({
  ...
  retention: {
    ...
    maxBytes: 1_000_000, // approximate byte budget (~1 MB)
  },
});
```

All three policies can be combined. When multiple policies apply, the one that
evicts the most events wins. Eviction always removes the oldest events first.
...
**`maxBytes`** uses a rough per-event estimate (64 bytes overhead + 8 per
number + 2×length per string + 4 per boolean). It's meant for order-of-magnitude
memory capping, not precise measurement.
