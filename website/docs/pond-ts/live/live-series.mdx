...
engine like Beam or Flink — pond-ts trades exactly-once semantics
and watermark/retraction handling for a simpler "data is the clock,
ordered ingest with bounded reorder tolerance" model. For the
specifics of what that does and doesn't cover once data leaves the
buffer, see
...
| manual eviction on push  | `retention: { maxEvents / maxAge }`            |
