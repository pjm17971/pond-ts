...
## Migration from `rolling.sample(seq)`

Pre-v0.12 pond exposed `LiveRollingAggregation.sample(sequence)` as
a separate method that emitted snapshots at sequence boundaries.
v0.12 deleted that method and folded the use case into the trigger
model:
...
const reported = live
  .rolling('1m', { cpu: 'p95' })
  .sample(Sequence.every('30s'));

// v0.12+
const reported = live.rolling(
  '1m',
  { cpu: 'p95' },
  { trigger: Trigger.every('30s') },
);
...
The new shape is one rolling with explicit cadence; the deprecated
shape was a rolling with a sub-sample. Functionally equivalent, but
the trigger version is more compositional (the cadence concept is
the same shape across event/clock/count) and cheaper (no separate
sub-aggregation deque).
