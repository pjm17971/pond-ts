...
The `PartitionedTimeSeries` view exposes sugar for the affected
operators — each runs the corresponding `TimeSeries` method
independently per partition and reassembles the output via
[`TimeSeries.concat`](#concat). The return type is always
`TimeSeries`, not the view, so chains break the partition by
design — re-`partitionBy` after to chain another partitioned op.
...
series.partitionBy('host').aggregate(Sequence.every('1m'), {
  cpu: 'avg',
  host: 'last', // keep host in the output for downstream filtering
});

// Composite partitioning — by host AND region
series.partitionBy(['host', 'region']).fill({ cpu: 'linear' });

// Custom composition via .apply()
series
  .partitionBy('host')
  .apply((g) => g.fill({ cpu: 'linear' }).rolling('5m', { cpu: 'avg' }));
```

**The escape hatch — `apply(fn)`.** For ops not exposed as sugar on
the view (or for arbitrary chained transforms within each partition),
`apply(fn)` runs `fn` on each partition and reassembles. The sugar
methods are themselves built on `apply`.

**On `groupBy` vs `partitionBy`.** Both partition by column value.
`groupBy(col)` returns a `Map<groupKey, TimeSeries<S>>` for callers
that want the per-group sub-series explicitly (e.g. to render N
separate charts). `partitionBy(col).<method>(...)` runs the
operation per-group and reassembles the output back into one
`TimeSeries<S>` — the inverse of `concat` happens for free. Reach
for `groupBy` when you want N outputs; for `partitionBy` when you
want one output with the per-partition computation correctly
scoped.
