...
- [**`groupBy(col, fn?)`**](#groupby) — partition a series by a
  column value into N per-group sub-series. (Fan-out.)
...
- [**`fromEvents(events, options)`**](#fromevents) — build a series
  from an existing `Event[]` array. Companion to `merge`.
...
`schema` (column-by-column on `name` and `kind`). Mismatches throw
upfront. For combining series with different schemas — joining CPU
metrics with memory metrics on time keys — use
