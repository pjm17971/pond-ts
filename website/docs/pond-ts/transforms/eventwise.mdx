...
# Eventwise Transforms

The simplest shape: events in, events out. One event per source event.
Same temporal key, sometimes a different schema. These are the
operators most pipelines start with.
...
first ([Sampling overview](./sampling-overview)), then shift by the
...
| Method                | Output                                     |
| --------------------- | ------------------------------------------ |
| `map(fn)`             | New schema possible; one event in, one out |
