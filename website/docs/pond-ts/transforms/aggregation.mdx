...
```ts
// "Most common tag per event" — count, then top1.
events.arrayAggregate('tags', 'count'); // tags: number column
events.arrayAggregate('tags', 'unique'); // tags: still array, deduped
```

Full treatment, including `{ as }` for appending vs replacing,
custom reducers, and the flatten-one-level semantics on array-kind
sources, lives on [Array columns](../data-shapes/arrays).
