...
:::note[`sample` option name vs the older `.sample()` method]

`align`'s `sample: 'begin' | 'center' | 'end'` option overlaps
verbally with the older `rolling.sample()` method that v0.12
...
a sequence boundary. For the streaming counterpart see
