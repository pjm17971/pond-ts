...
(per-event by default for live, per-bucket for batch). When you want
to **report** the rolling state at a regular cadence (e.g. push p95
to a backend every 30 s), set a `Trigger.every(duration)` on the
rolling:
...
source event crosses an epoch-aligned boundary of the underlying
sequence. The duration must be a fixed-step value. See
