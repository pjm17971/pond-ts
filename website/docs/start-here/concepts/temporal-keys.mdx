...
- **Schema first-column declaration** — the
  ...
  construction. See [Ingest](../../pond-ts/transforms/ingest).
- **Aggregation and alignment** — `aggregate(seq, mapping)` and
  ...
  [Alignment](../../pond-ts/transforms/sampling-overview).
- **Selection vocabulary** — operations like `tail`, `between`,
  `within`, `overlapping`, `trim` all work in terms of `begin()` /
  `end()`. See
  [Selection vocabulary](../concepts#selection-vocabulary).
