...
produces one Interval-keyed event per bucket. Buckets are
half-open [start, end), so an event at exactly the boundary lands
in the later bucket](/img/concepts-sequence.png)
