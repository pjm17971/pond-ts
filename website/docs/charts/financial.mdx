...
Twenty studies, every one oracle-verified (pandas, and TA-Lib where TA-Lib
has the indicator) and each a batch transform (`TimeSeries → TimeSeries +
columns`). Grouped by where they are drawn:
...
| `vwap`      | `vwap`                                | Rolling volume-weighted typical price, `Σ tp·v / Σ v`.          |
...
`kPeriod + slowing − 2` for `%K`; and a **flat window is `undefined`**, not
...
auto-scaled axis around a zero baseline:
