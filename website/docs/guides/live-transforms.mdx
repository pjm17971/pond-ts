...
With `grace: '5s'`, a bucket doesn't finalize until the data's watermark
(highest timestamp seen) advances 5 seconds past the bucket's end. During that
window:
...
Without a `grace` option (the default), buckets close immediately when an event
crosses the boundary — identical to the behavior before grace was introduced.

**Tip:** pair `grace` on the aggregation with `ordering: 'reorder'` on the
source `LiveSeries` so the source accepts late events instead of throwing.
