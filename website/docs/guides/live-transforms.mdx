...
## Rolling: sliding-window reduction

`Rolling` maintains a sliding-window aggregate that updates on every source
...
`Rolling` also satisfies `LiveSource`. Each source event produces an output
event containing the rolling aggregate at that point, preserving the source
event's timestamp:

```ts
const rolling = new Rolling(live, '5m', { cpu: 'avg' });
...
This makes `Rolling` the live equivalent of the batch `rolling()` method. The
