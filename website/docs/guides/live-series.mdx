...
`LiveSeries` is a mutable, append-optimized buffer that shares the same schema
type system as `TimeSeries`. Use it when data arrives incrementally — from
WebSockets, message queues, polling loops, or any streaming source — and you
need to maintain a rolling window of recent events.
