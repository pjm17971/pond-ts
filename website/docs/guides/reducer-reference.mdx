...
const result = series.reduce({
  ...
  host: 'unique', //   -> ReadonlyArray<ScalarValue> | undefined
  paths: 'top3', //   -> ReadonlyArray<ScalarValue> | undefined
  status: 'last', //   -> string | undefined  (source column kind)
});

const avgCpu: number | undefined = result.cpu;
const hosts: ReadonlyArray<string | number | boolean> | undefined = result.host;
```
