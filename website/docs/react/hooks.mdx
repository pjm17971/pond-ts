...
function Chart({ data }: { data: TimeSeriesJsonPayload }) {
  ...
  return <LineChart points={series.toPoints('cpu')} />;
}
```
...
function Stats({ live }: { live: LiveSeries<Schema> | null }) {
  ...
  return <Chart points={snap.toPoints('cpu')} />;
}
```
...
function CpuAverages({ snap }: { snap: TimeSeries<Schema> | null }) {
  ...
  return hourly && <Chart data={hourly.toPoints('cpu')} />;
}
```
...
Full generated API reference for every signature: [/api](/api) (pick
`@pond-ts/react`).
