...
[`packages/core/test/error-rate-dashboard.test.ts`](https://github.com/pjm17971/pond-ts/blob/main/packages/core/test/error-rate-dashboard.test.ts)
...
```ts
const broadOutages = withCount.filter(
  (e) => (e.get('hostCount') as number) >= 3,
);
...
4. **The open bucket isn't reachable via `.at()` or `.length`.**
   ...
   event (see the
   [Live Transforms guide → Reading the current value](../guides/live-transforms#reading-the-current-value)).
...
function renderDashboard() {
  ...
  renderAlerts(perMinute.filter((e) => (e.get('hostCount') as number) >= 3));
...
[`useCurrent`](../guides/live-transforms#in-react-usecurrent):
...
See the
[Live Transforms → Reading the current value](../guides/live-transforms#reading-the-current-value)
