...
| `centerOfGravity`           | `cog`                                                                      | `{ period?, column?, output? }` (Ehlers' position-weighted balance point, default 10; **negative**, in `[−period, −1]`, flat reads `−(period+1)/2` — TradingView's uncentred convention)                                            | `packages/financial/src/studies/center-of-gravity.ts`          |
