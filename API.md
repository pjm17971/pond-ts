...
| `highLowBands`              | `hlbMiddle`, `hlbUpper`, `hlbLower`                                        | `{ period?, percent?, maType?, high?, low?, prefix? }` (MA(**median price**) × (1 ± shift%), 10 / 1% / trima; `shift` is `envelope`'s `percent` under this indicator's own label)                                                                | `packages/financial/src/studies/high-low-bands.ts`             |
| `bollingerBandwidth`        | `bbWidth`                                                                  | `{ period?, stdDev?, column?, output? }` (`100·(upper − lower)/middle`, 20 / 2 — the **×100** StockCharts form; a flat window is `0`, not missing)                                                                                               | `packages/financial/src/studies/bollinger-derived.ts`          |
