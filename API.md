...
| `directionalMovement`       | `dmPlusDi`, `dmMinusDi`, `dmDx`, `dmAdx`, `dmAdxr` | `{ period?, high?, low?, close?, prefix? }` (Wilder's DMS — `+DI`/`−DI`/`DX`/`ADX`/`ADXR`, default 14; per-column warm-up; Wilder's seed, so a decaying transient vs TA-Lib) | `packages/financial/src/studies/directional-movement.ts`       |
