...
describe('Event', () => {
  it('exposes typed payload access and immutable updates', () => {
    const event = new Event(new Time(1000), {
      cpu: 0.5,
      host: 'api-1',
      healthy: true,
    });
    ...
    expect(event.select('cpu', 'healthy').data()).toEqual({
      cpu: 0.5,
      healthy: true,
    });
    expect(event.rename({ cpu: 'usage' }).data()).toEqual({
      usage: 0.5,
      host: 'api-1',
      healthy: true,
    });
  ...
  it('supports select, rename and collapse while preserving the key', () => {
    ...
    expect(event.select('in', 'healthy').data()).toEqual({
      in: 10,
      healthy: true,
    });
  ...
  it('supports temporal delegation and trim', () => {
    const event = new Event(new TimeRange({ start: 1000, end: 2000 }), {
      value: 1,
    });

    expect(event.overlaps(new TimeRange({ start: 1500, end: 2500 }))).toBe(
      true,
    );
    expect(event.contains(new Time(1500))).toBe(true);
    expect(
      event.intersection(new TimeRange({ start: 1500, end: 2500 })),
    ).toEqual(new TimeRange({ start: 1500, end: 2000 }));
    expect(
      event.trim(new TimeRange({ start: 1500, end: 2500 }))?.key(),
    ).toEqual(new TimeRange({ start: 1500, end: 2000 }));
  });

  it('supports converting event key types', () => {
    const rangeEvent = new Event(new TimeRange({ start: 1000, end: 2000 }), {
      value: 1,
    });
    ...
    expect(rangeEvent.asTimeRange().key()).toEqual(
      new TimeRange({ start: 1000, end: 2000 }),
    );
    expect(intervalEvent.key()).toEqual(
      new Interval({ value: 'bucket-a', start: 1000, end: 2000 }),
    );
