...
describe('TimeRange', () => {
  ...
  it('supports containment, overlap, intersection and trim', () => {
    ...
    expect(outer.intersection(overlap)).toEqual(
      new TimeRange({ start: 2500, end: 3000 }),
    );
  ...
  it('builds local-day and calendar ranges from string references', () => {
    ...
    expect(
      TimeRange.fromCalendar('week', '2025-01-01', {
        timeZone: 'UTC',
        weekStartsOn: 1,
      }),
    ).toEqual(
    ...
    expect(
      TimeRange.fromCalendar('month', '2025-01', { timeZone: 'UTC' }),
    ).toEqual(
      new TimeRange({
