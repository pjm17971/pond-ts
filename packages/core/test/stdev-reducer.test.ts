 ...
 * construction, matching the two-pass `reduce` path to floating-point noise.
 */
...
// The two-pass row-API reduce — the parity reference bucketState must match.
function twoPass(values: number[]): number | undefined {
  return stdev.reduce(values, values) as number | undefined;
}

describe('stdev.bucketState — Welford online variance (audit §1.1)', () => {
  it('matches the two-pass reduce on ordinary data', () => {
    const xs = [2, 4, 4, 4, 5, 5, 7, 9]; // textbook population stdev = 2
    expect(bucket(xs)).toBeCloseTo(2, 12);
    expect(bucket(xs)).toBeCloseTo(twoPass(xs)!, 12);
  });
  ...
  it('does not cancel on near-equal large-magnitude values (was 0)', () => {
    ...
    expect(bucket(xs)).toBeCloseTo(twoPass(xs)!, 9);
  });
