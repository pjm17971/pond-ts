...
> implements LiveSource<Out> {
   ...
   * is the next index to remove on evict. The i-th evicted event
   * (oldest-first) removes index `#nextEvictIdx + i` using that
   * event's data value.
   *
   * This replaced an earlier `WeakMap<Event, absIdx>` keyed by source
   * event reference. That assumed the `'evict'` callback delivered
   * the SAME `Event` object the `'event'` callback delivered — true
   * for the row-backed source (it stored the object) but NOT for the
   * ring-backed source (it materializes events lazily, so an evicted
   * event is a fresh object with equal value but different identity).
   * FIFO position is identity-independent and correct for both
   * backings, since retention is always oldest-first.
   */
  #nextAbsIdx: number;
  #nextEvictIdx: number;
   ...
   *   `LiveReduce` (already evicted past the FIFO cursor) don't
  ...
  #evictOne(event: EventForSchema<S>): void {
    if (this.#disposed) return;
    // FIFO: the oldest live event leaves next. If everything added
    // has already been evicted, this is an evict for an event that
    // predated this LiveReduce (or a double-evict) — skip, matching
    // the prior WeakMap-miss guard.
